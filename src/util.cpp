...
mvmcts::MvmctsParameters MakeMctsParameters(const commons::ParamsPtr& params) {
  ...
  mcts_p.thres_uct_statistic_.THRESHOLD = StdToEigen(params->GetListFloat(
      "BehaviorRulesMcts::Threshold", "Thresholds for improving lower level goals", std::vector<float>(mcts_p.REWARD_VEC_SIZE, std::numeric_limits<float>::max())));
  assert(mcts_p.thres_uct_statistic_.THRESHOLD.size() == reward_vec_size);

  mcts_p.thres_uct_statistic_.EPSILON = params->GetReal("BehaviorRulesMcts::EpsilonGreedy",
