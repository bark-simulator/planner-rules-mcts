...
#include "bark/models/tests/make_test_world.hpp"
#include "bark/world/evaluation/evaluator_collision_ego_agent.hpp"
#include "bark/world/evaluation/evaluator_goal_reached.hpp"
#include "bark/world/evaluation/ltl/labels/generic_ego_label_evaluator.hpp"
#include "bark/world/goal_definition/goal_definition_polygon.hpp"
#include "bark/world/goal_definition/goal_definition_state_limits.hpp"

using namespace bark::models::behavior;
using namespace mcts;
using bark::models::tests::make_test_observed_world;
using bark::models::tests::make_test_world;
...
using bark::world::evaluation::GenericLabelEvaluator;
using bark::world::evaluation::GenericEgoLabelEvaluator;
using bark::world::goal_definition::GoalDefinitionStateLimits;
using bark::world::prediction::PredictionSettings;
using bark::world::goal_definition::GoalDefinitionStateLimits;
using bark::world::prediction::PredictionSettings;
...
LabelEvaluators make_labels() {
  ...
  labels.emplace_back(new GenericEgoLabelFunction<EvaluatorCollisionEgoAgent>(
      "collision_ego"));
  labels.emplace_back(
...
TEST(single_agent_mcts_state, execute) {
  ...
  MvmctsStateMultiAgent mcts_state(observed_world, make_rule_states(make_rules()), &state_params, {1}, 20, &label_evaluators);
  ...
  EXPECT_FALSE(next_mcts_state->IsTerminal()); // < make test world is defined in such a way that
  ...
  EXPECT_TRUE(next_mcts_state->IsTerminal()); // < crazy action should lead to a collision with driving corridor
  rewards += next_mcts_state->GetTerminalReward();
  ...
  EXPECT_TRUE(next_mcts_state->IsTerminal()); // < action 3 should lead to a collision with other agent
  rewards += next_mcts_state->GetTerminalReward();
  EXPECT_NEAR(rewards[0](0), -1000 , 0.00001);
...
TEST(single_agent_mcts_state, execute_goal_reached_state_limits) {
  ...
  MvmctsStateParameters state_params(&params);
  MvmctsStateMultiAgent mcts_state(observed_world, make_rule_states(make_rules()), {1}, 20, &state_params, &labels);
...
//  BehaviorUCTMultiAgent behavior_uct(&params);
...
//
//  Trajectory trajectory = behavior_uct.Plan(prediction_time_span, observed_world);
//  // According to the default motion primitives the best action should be to brake to avoid crashing the other agent
